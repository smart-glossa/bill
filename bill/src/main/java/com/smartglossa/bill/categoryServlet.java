	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (operation.equals("add")) {
			...
			try {
				...
				String query = "insert into expense(catid,catname)values(" + id + ",'" + name + "')";
				stat.execute(query);
				result.put("status", 1);
			} catch (Exception e) {
				result.put("status", 0);
			...
			try {
				...
				String query = "update expense set catname='" + name + "' where catid=" + id;
				stat.execute(query);
				res.put("status", 1);
			...
			try {
				...
				String query = "delete from expense where catid=" + cid;
				state.execute(query);
				ress.put("status", 1);
			...
			try {
				...
				String query = "select * from expense where  catid=" + caid;
