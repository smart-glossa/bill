...
public class SaleMetaDataClass {
    ...
    public void updateSaleMetaData(int saleId, String billDate, float vat, float discount, float billTotal)
            throws SQLException {
        try {
            String query = "update salemetadata set billDate='" + billDate + "',vat=" + vat + ",discount=" + discount
                    + ",billTotal=" + billTotal + " where saleId=" + saleId;
            stmt.execute(query);
        } finally {
            closeConnection();
        }
